...
public class ExcelReader {
	...
	public long importData(IContext context, IMendixObject fileDocument, IMendixObject templateObject, IMendixObject parentObject)
		...
		final HashMap<String, String> sortMap = new HashMap<>();
		sortMap.put(Column.MemberNames.ColNumber.toString(), "ASC");

		final List<IMendixObject> columns = Core.retrieveXPathQuery(
				settings.getContext(), "//" + Column.getType() + "[" + Column.MemberNames.Column_Template + "=" + templateObject.getId().toLong() + "]",
				Integer.MAX_VALUE,
				0,
				sortMap);
		for (IMendixObject columnObject : columns) {
			...
			if (type == MappingType.Attribute) {
				...
				final IMendixObject assocationObject = Core.retrieveId(settings.getContext(), associationId);
				...
				final String associationName = assocationObject.getValue(settings.getContext(), MxObjectReference.MemberNames.CompleteName.toString());
		...
		final List<IMendixObject> refHandlingList = Core.retrieveXPathQuery(
			settings.getContext(), "//" + ReferenceHandling.getType() + "[" + ReferenceHandling.MemberNames.ReferenceHandling_Template + "=" + templateObject.getId().toLong() + "]");
		for (IMendixObject object : refHandlingList) {
			...
			final String selecteReferenceHandling = object.getValue(settings.getContext(), ReferenceHandling.MemberNames.Handling.toString());

			final AssociationConfig config = settings.getAssociationConfig(associationName);

			switch (ReferenceHandlingEnum.valueOf(selecteReferenceHandling)) {
				case FindCreate:
		...
		final IMendixIdentifier parentAssocationId = templateObject.getValue(settings.getContext(), Template.MemberNames.Template_MxObjectReference_ParentAssociation.toString());
		if (parentAssocationId != null) {
			final IMendixObject parentAssocation = Core.retrieveId(settings.getContext(), parentAssocationId);
			final String associationName = parentAssocation.getValue(settings.getContext(), MxObjectReference.MemberNames.CompleteName.toString());
		...
		try {
			...
			switch (getExcelExtension(settings.getContext(), fileDocument)) {
				...
				case XLSX:
				case XLSM:
					final String xpath = String.format("count(//%s[%s = %d])", Column.entityName, Column.MemberNames.Column_Template, templateObject.getId().toLong());
					logNode.debug(xpath);

					final Long columnCount = Core.retrieveXPathQueryAggregate(settings.getContext(), xpath);
